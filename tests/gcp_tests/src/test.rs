...
fn is_gcp_cvm() {
    if is_sev() {
        println!("Running in GCP AMD SEV environment");
    } else if is_tdx() {
        println!("Running in GCP Intel TDX environment");
    } else {
        panic!("Not running in a supported GCP CVM environment");
    }
}

#[test]
fn get_and_parse_gcp_quote() {
