...
pub fn get_pck_crl(pccs_url: &str, ca: PckCa) -> Result<(Vec<u8>, Vec<u8>), Error> {
    ...
    let pck_crl = hex::decode(body).map_err(|e| Error::DecodeError(e.to_string()))?;

    Ok((pck_crl_issuer_chain, pck_crl))
}
...
pub fn get_tcbinfo(pccs_url: &str, fmscp: &[u8]) -> Result<(Vec<u8>, Vec<u8>), Error> {
    let url = reqwest::Url::parse_with_params(
        &format!("{pccs_url}/sgx/certification/v4/tcb"),
...
pub fn get_qe_identity(pccs_url: &str) -> Result<(Vec<u8>, Vec<u8>), Error> {
    let url = reqwest::Url::from_str(&format!("{pccs_url}/sgx/certification/v4/qe/identity"))
        .map_err(|e| Error::URLError(e.to_string()))?;
...
mod tests {
    ...
    fn test_tcb_info() {
        ...
        let (tcb_info_issuer_chain, tcb_info) =
            get_tcbinfo("https://pccs.mse.cosmian.com", &fmspc).unwrap();
    ...
    fn test_qe_identity() {
        let (qe_identity_issuer_chain, qe_identity) =
            get_qe_identity("https://pccs.mse.cosmian.com").unwrap();
